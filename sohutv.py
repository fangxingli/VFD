...
def getRootMenu():
	...
	u=argv[0]+"?mode=10&name="+urllib.quote_plus(name)
	t.addDirectoryItem(int(argv[1]),u,li,True)
	for name, id in CHANNEL_LIST:
		li=util.MenuItem(name)
		u=argv[0]+"?mode=1&name="+urllib.quote_plus(name)+"&id="+urllib.quote_plus(id)+"&cat="+urllib.quote_plus("")+"&area="+urllib.quote_plus("")+"&year="+urllib.quote_plus("-1")+"&order="+urllib.quote_plus("0")+"&page="+urllib.quote_plus("1")+"&p5="+urllib.quote_plus("")+"&p6="+urllib.quote_plus("")+"&p11="+urllib.quote_plus("")
		t.addDirectoryItem(int(argv[1]),u,li,True)
...
def progList(name,id,page,cat,area,year,p5,p6,p11,order):
	...
	if match[0]=='0':
		...
		if id!='13':
			...
			'''
			#筛选换分类
			li = util.MenuItem(name+'（第'+str(currpage)+'/'+str(totalpages)+'页）【' + lxstr + '/[COLOR FF00FFFF]' + searchDict(ORDER_LIST,order) + '[/COLOR]】（按此选择）')
			u = argv[0]+"?mode=4&name="+urllib.quote_plus(name)+"&id="+id+"&cat="+urllib.quote_plus(cat)+"&area="+urllib.quote_plus(area)+"&year="+urllib.quote_plus(year)+"&order="+order+"&listpage="+urllib.quote_plus(listpage)+"&p5="+urllib.quote_plus(p5)+"&p6="+urllib.quote_plus(p6)+"&p11="+urllib.quote_plus(p11)
		...
		for i in range(0,len(match)):
			...
			u = argv[0]+"?mode="+str(mode)+"&name="+urllib.quote_plus(p_name)+"&url="+urllib.quote_plus(p_url)+"&thumb="+urllib.quote_plus(p_thumb)+"&id="+urllib.quote_plus(str(i))
			...
			t.addDirectoryItem(int(argv[1]), u, li, p_dir, totalItems)

		if currpage > 1:
			li = util.MenuItem('上一页')
			u = argv[0]+"?mode=1&name="+urllib.quote_plus(name)+"&id="+id+"&cat="+urllib.quote_plus(cat)+"&area="+urllib.quote_plus(area)+"&year="+urllib.quote_plus(year)+"&order="+order+"&page="+urllib.quote_plus(str(currpage-1))
			t.addPageItem(u, li)
...
def seriesList(name,id,url,thumb):
	...
	if url.find('.shtml')>0:
		...
		for p_thumb,p_url,p_name in match:
			li = util.MenuItem('第'+p_name+'集', iconImage = '', thumbnailImage = p_thumb)
			li.bindMedia(util.Media())
			u = argv[0] + "?mode=3&name=" + urllib.quote_plus('第'+p_name+'集') + "&url=" + urllib.quote_plus(p_url)+ "&thumb=" + urllib.quote_plus(p_thumb)
		...
		if len(match0)>0:
			...
			for item in items:
				...
				if len(href)>0:
					...
					li = util.MenuItem(p_name, iconImage = p_thumb, thumbnailImage = p_thumb)
					li.bindMedia(util.Media())
					u = argv[0] + "?mode=3&name=" + urllib.quote_plus(p_name) + "&url=" + urllib.quote_plus(p_url)+ "&thumb=" + urllib.quote_plus(p_thumb)
...
def LiveChannel(name):
	...
	if match:
		parsed_json = json.loads(match[0])
		...
		for item in parsed_json['data']:
			...
			li = util.MenuItem(p_name, iconImage = '', thumbnailImage = p_thumb)
			u = argv[0] + "?mode=11&name=" + urllib.quote_plus(p_name) + "&id=" + urllib.quote_plus(id)+ "&thumb=" + urllib.quote_plus(p_thumb)
			t.addDirectoryItem(int(argv[1]), u, li, False, totalItems)
	t.setContent(int(argv[1]), 'movies')
...
def decode(code):
	...
	if mode == None:
		...
		return progList(name,id,page,cat,area,year,p5,p6,p11,order)
	elif mode == 2:
		return seriesList(name,id,url,thumb)
	elif mode == 3:
		return PlayVideo(name,url,thumb)
	elif mode == 4:
		return performChanges(name,id,listpage,cat,area,year,order,p5,p6,p11)
	elif mode == 5:
		return PlayBoKe(name,url,thumb)
	elif mode == 10:
