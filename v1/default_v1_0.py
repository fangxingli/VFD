...
import io
#from phony import LevelTree
import phony as xbmc
import phony as xbmcgui
import phony as xbmcplugin
import phony as xbmcaddon
...
def progList(name,type,cat,page):
	...
	xbmc.ImageCache('/workspace/python-movie/src/imgs', [ i.getURLImage() for i in t.getItems() ]).start()
	print '**********************' + xbmc.IMG_PATH
	# For Test
...
def ListB(name,url,thumb):
    ...
    for i in range(len(vidlist)):
        ...
        xbmcplugin.addDirectoryItem(int(argv[1]), u, li, False, len(vidlist))
