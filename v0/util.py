...
class Categorization(object):
	...
	def addCategorization(self, cate_name, alias, cate_map = {}):
		if len(cate_map) != 0:
			self.cate_table[(cate_name, alias)] = cate_map

	def getCate(self, cate_name_alias):
		for i in self.cate_table.keys():
			if cate_name_alias in i:
				return self.cate_table[i]
		raise IndexError, '%s not exists in cate table' %(cate_name_alias)

	def getCateSize(self):
		return len(self.cate_table)

	def getGivenCateSize(self, cate_name_alias):
