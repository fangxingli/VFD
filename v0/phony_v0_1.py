...
class ImageCache:
	...
	def checkPathValid(self):
		if not os.path.exists(IMG_PATH):
			raise IOError, '    Path %s does not exists' % (IMG_PATH)
	...
	def localize(self):
		...
		for i in self.imgs:
			...
			if i != '':
				...
				data = response.read()
				self.urls.append(IMG_PATH+i[i.rfind('/'):])
	#			print self.urls[-1]
				f = io.open(self.urls[-1], 'wb')
				f.write(data)
				f.close()
			else:
				print i+ '------------------------------'
...
class ListItem:
	...
	def getLocalImage(self):
		print IMG_PATH + self.iconImage[self.iconImage.rfind('/'):]
		return IMG_PATH + self.iconImage[self.iconImage.rfind('/'):]
	def getLabel(self):
		return self.label
	def __str__(self):
		return self.label
...
class LevelTree:
	...
	def endOfDirectory(self, handle, succeeded=True, updateListing=True, cacheToDisc=True):
		...
		return self.selectItem(self.items[getin])
